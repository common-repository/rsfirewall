        ...
        switch ( $column ) {
            ...
            case 'actions':
                if ($status = get_post_status($post_id)) {
                    ...
                    {
                        $value = '<a href="' . wp_nonce_url($edit_url, 'rsfirewall', 'rsf-actions') . '&handler=feeds&task=change_status&id=' . $post_id . '" class="rsfirewall-btn small" type="button" id="rsf-list-status" data-pid="' . $post_id . '">' . __('Publish', 'rsfirewall') . '</a>';
                    } 
                    else
                    {
